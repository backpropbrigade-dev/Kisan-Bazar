...
exports.createProduct = async (req, res) => {
  try {
    ...
    // Check Farmer SaaS Tier product limits
    const farmerProfile = await FarmerProfile.findOne({ user: req.user._id });
    const plan = farmerProfile?.subscriptionPlan || "free";
    ...
    if (plan === "free") {
      ...
      if (productCount >= 5) {
        return res.status(403).json({
          success: false,
          message: "Free plan limit reached (5 products max). Upgrade to Pro Plan for unlimited produce listings!",
